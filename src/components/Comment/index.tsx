...
import { pickDataAttributes } from "@/utils/pick";
import "./index.less";
const defaultAlign = {
  datetime: "left",
  actions: "left",
...
export function Comment(baseProps: PropsWithChildren<CommentProps>, ref) {
  const { getPrefixCls, componentConfig, rtl } = useContext(ConfigContext);
  const props = useMergeProps<PropsWithChildren<CommentProps>>(baseProps, defaultProps, componentConfig?.Comment);
    ...
    >
      ...
      <div className={`${prefixCls}-inner`}>
        <div className={`${prefixCls}-inner-content`}>
          ...
          {actions && (
            <div className={`${prefixCls}-actions ${prefixCls}-actions-align-${align.actions}`}>{actions}</div>
          )}
        </div>
